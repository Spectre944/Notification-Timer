...
PopUp::PopUp(QWidget *parent) : QWidget(parent)
{
    ...
    //popup_icon = QIcon("://images/exercise.png");
...
void PopUp::paintEvent(QPaintEvent *event)
{
    ...
    painter.setBrush(QBrush(QColor(140,140,140,140)));
    painter.setPen(Qt::NoPen);

    painter.drawRoundedRect(roundedRect, 10, 10);
}

void PopUp::setPopupTextAndImage(const QString &text, const QImage &image)
{
    label.setText(text);    // Set the text in the Label

    inputImg = new QImage(image);

    QImage small = inputImg->scaled( 65, 65, Qt::KeepAspectRatio);

    label_img.setPixmap(QPixmap::fromImage(small));
...
void PopUp::show()
{
    ...
    setGeometry(QApplication::desktop()->availableGeometry().width()  - width() + QApplication::desktop() -> availableGeometry().x(),
                QApplication::desktop()->availableGeometry().height() - 170 - height() + QApplication::desktop() -> availableGeometry().y(),
                width(),
                height());
    QWidget::show();
